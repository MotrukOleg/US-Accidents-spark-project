...
def om_requests(olap):
    analysis_functions = [
        ("extreme_delay_anomalies", get_extreme_delay_anomalies),
        ("severity_by_weather", get_severity_by_weather),
        ("top_dangerous_hours_per_state", get_top_dangerous_hours_per_state),
        ("hourly_trend_by_state", get_hourly_trend_by_state),
        ("night_accident_percentage", get_night_accident_percentage),
        ("infrastructure_contribution_by_state", get_infrastructure_contribution_by_state)
    ]
    ...
    for i, (file_name, func) in enumerate(analysis_functions, start=1):
        ...
        save_results(df_result, file_name, "oleh_motruk")
...
def get_top_dangerous_hours_per_state(olap):
    ...
    return (
        state_hour_stats
        .withColumn("Rank", F.dense_rank().over(window_rank))
        .filter(F.col("Rank") <= 3)
        .orderBy("State", "Rank")
    )
...
def get_night_accident_percentage(olap):
    ...
    return (
        night_by_city
        .join(total_by_city, ["State", "City"])
        .withColumn(
            "Night_Ratio",
            F.round((F.col("Night_Accidents") / F.col("Total_Accidents")) * 100, 2)
        )
...
def get_infrastructure_contribution_by_state(olap):
    ...
    contribution_df = base_df.groupBy("State").agg(
        ...
        *[
            F.sum(F.when(F.col(c) == True, 1).otherwise(0)).alias(c)
...
def get_extreme_delay_anomalies(olap):
    df = olap["accidents_fact"].join(
        F.broadcast(olap["location_dim"]), "LocationID"
    )
    ...
    return (
        df
        .withColumn("City_Avg_Duration", F.avg("Duration_Minutes").over(city_window))
